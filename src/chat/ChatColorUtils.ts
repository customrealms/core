...
export class ChatColorUtils {
    ...
    public static parseColorsToMinecraft(str: string): string {
        for (let find in ChatColorUtils.COLORCODE_TO_HEX) {
            str = str.replace(new RegExp(find, 'g'), '\u00A7' + find.substr(1));
        }
        for (let find in ChatColorUtils.ATTRIBUTE_CODES) {
            str = str.replace(new RegExp(find, 'g'), '\u00A7' + find.substr(1));
        }
        return str;
    }
    ...
    public static parseColorsToHtml(str: string): string {
        return this
            ...
            .map(part => {
                ...
                if (part.attrs.includes('italic')) styles['font-style'] = 'italic';
                ...
                const styles_str: string = Object
                    .keys(styles)
    ...
    private static parseColorsToParts(str: string): IPartStyle[] {
        ...
        str
            ...
            .forEach((part: string, index: number) => {
                ...
                const color_val: string | undefined = ChatColorUtils.COLORCODE_TO_HEX[`&${code}`];
                const attr_val: string | undefined = ChatColorUtils.ATTRIBUTE_CODES[`&${code}`];
