...
export class Inventory implements ToJava {
     ...
     */
    public contains(material_or_item: Material | ItemStack, amount?: number): boolean {
        if (material_or_item instanceof Material) {
            if (typeof amount === 'number') return this.toJava().contains(
     ...
     */
    public remove(material_or_item: Material | ItemStack, amount?: number): void {
        if (material_or_item instanceof Material) {
            if (typeof amount === 'number') this.toJava().remove(
                material_or_item.toJava(),
                amount
            );
