...
export class CommandRunner {
    ...
    public attemptExecute(player: Player, message: string): boolean {
        ...
        const call: CommandCall | null = this.getMatchingCommandCall(player, message);
    ...
    private getMatchingCommandCall(player: Player, message: string): CommandCall | null {
        ...
        for (let i = 0; i < this.variations.length; i++) {
            ...
            const match: RegExpMatchArray | null = message.match(variation.regex!);
            if (!match) continue;
